...
import com.naddame.dto.KeyAndPassword;
import com.naddame.dto.ManagedUser;
import com.naddame.dto.UserDTO;
import com.naddame.model.User;
import com.naddame.repository.UserRepository;
import com.naddame.security.SecurityUtils;
...
public class AccountResource {
    ...
    public ResponseEntity<?> registerAccount(@Valid @RequestBody ManagedUser managedUser) {

        HttpHeaders textPlainHeaders = new HttpHeaders();
        textPlainHeaders.setContentType(MediaType.TEXT_PLAIN);

        return userRepository.findOneByLogin(managedUser.getLogin().toLowerCase())
            .map(user -> new ResponseEntity<>("login already in use", textPlainHeaders, HttpStatus.BAD_REQUEST))
            .orElseGet(() -> userRepository.findOneByEmail(managedUser.getEmail())
                .map(user -> new ResponseEntity<>("e-mail address already in use", textPlainHeaders, HttpStatus.BAD_REQUEST))
                .orElseGet(() -> {
                    User user = userService
                        .createUser(managedUser.getLogin(), managedUser.getPassword(),
                            managedUser.getFirstName(), managedUser.getLastName(),
                            managedUser.getEmail().toLowerCase(), managedUser.getLangKey());
