...
import com.naddame.service.util.RandomUtil;
...
public class UserService {
    ...
    public User createUser(ManagedUser managedUser) {
        User user = new User();
        user.setLogin(managedUser.getLogin());
        user.setFirstName(managedUser.getFirstName());
        user.setLastName(managedUser.getLastName());
        user.setEmail(managedUser.getEmail());
        if (managedUser.getLangKey() == null) {
            user.setLangKey("en"); // default language
        } else {
            user.setLangKey(managedUser.getLangKey());
        }
        if (managedUser.getAuthorities() != null) {
            Set<Authority> authorities = new HashSet<>();
            managedUser.getAuthorities().forEach(
    ...
    public void removeNotActivatedUsers() {
        ...
        List<User> users = userRepository.findAllByActivatedIsFalseAndCreatedBefore(now.minusDays(3));
        for (User user : users) {
